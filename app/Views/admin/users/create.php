...
<div class="max-w-3xl mx-auto">
    <div class="bg-white rounded-2xl shadow-lg shadow-slate-200/50 border border-slate-100 overflow-hidden">
        <div class="p-4 sm:p-6 border-b border-slate-100">
            <h2 class="text-lg font-bold text-slate-900">Tambah Pengguna Baru</h2>
            <p class="text-sm text-slate-500 mt-1">Isi detail pengguna yang akan ditambahkan</p>
        </div>
        
        <form action="<?= base_url('admin/users/store') ?>" method="POST" class="p-4 sm:p-6">
            <?= csrf_field() ?>
                ...
                <div>
                    ...
                    <div class="bg-slate-50 rounded-xl p-4 space-y-3">
                        ...
                        <label class="flex items-center space-x-3 cursor-pointer p-2 -mx-2 rounded-lg hover:bg-slate-100 transition-colors">
                            <input type="checkbox" name="roles[]" value="<?= $role['id'] ?>" <?= old('roles') && in_array($role['id'], old('roles')) ? 'checked' : '' ?>
